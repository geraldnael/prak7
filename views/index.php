  ...
  <div class="modal fade" id="addTodo" tabindex="-1" aria-labelledby="addTodoLabel" aria-hidden="true">
    <div class="modal-dialog">
      <div class="modal-content">
        ...
        <form action="?page=update" method="POST" onsubmit="return validateEditActivity();">
...
<script>
  // Fungsi ini akan dipanggil saat tombol "Save" pada modal "Tambah Data" ditekan.
  function validateDuplicateActivity() {
    const inputActivity = document.getElementById("inputActivity").value;

    // Mengambil semua aktivitas yang ada dalam tabel
    const existingActivities = Array.from(document.querySelectorAll("td:nth-child(2)")).map(td => td.textContent.trim());

    // Periksa apakah aktivitas yang dimasukkan sudah ada dalam daftar
    if (existingActivities.includes(inputActivity)) {
      alert("Aktivitas sudah ada dalam daftar!");
      return false; // Menghentikan pengiriman formulir
    }

    // Jika tidak ada duplikasi, formulir akan terus di-submit
    return true;
  }

  // Fungsi ini akan dipanggil saat tombol "Save" pada modal "Ubah Data" ditekan.
  function validateEditActivity() {
    const inputEditActivity = document.getElementById("inputEditActivity").value;
    
    // Mengambil semua aktivitas yang ada dalam tabel
    const existingActivities = Array.from(document.querySelectorAll("td:nth-child(2)")).map(td => td.textContent.trim());

    // Mengambil aktivitas yang sedang diedit (jika ada)
    const editedActivity = inputEditActivity.trim();

    // Periksa apakah aktivitas yang dimasukkan sudah ada dalam daftar, kecuali saat mengedit aktivitas itu sendiri
    if (existingActivities.includes(editedActivity) && existingActivities.indexOf(editedActivity) === -1) {
      alert("Aktivitas sudah ada dalam daftar!");
      return false; // Menghentikan pengiriman formulir
    }
